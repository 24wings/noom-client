...
export class AppUrlService {
  ...
  private static ensureEndsWith(str: string, c: string) {
  ...
  private static removeFromEnd(str: string, c: string) {
    if (str.charAt(str.length - 1) === c) {
  ...
  private static removeFromStart(str: string, c: string) {
    if (str.charAt(0) === c) {
  ...
  getAppRootUrlOfTenant(tenancyName?: string): string {
    let baseUrl = AppUrlService.ensureEndsWith(AppConsts.appBaseUrlFormat, '/');

    // Add base href if it is not configured in appconfig.json
    if (baseUrl.indexOf(AppConsts.appBaseHref) < 0) {
      baseUrl = baseUrl + AppUrlService.removeFromStart(AppConsts.appBaseHref, '/');
    }
