...
export abstract class PagedListingComponentBase<EntityDto> extends AppComponentBase implements OnInit {
  ...
  change(e: STChange) {
    ...
    if (e.type === 'sort') {
      this.sorting = e.sort.column.index[e.sort.column.index.length - 1] + ' ' + (e.sort.value === 'ascend' ? 'ASC' : 'DESC');
      this.refresh();
    }
  }
   ...
   */
  protected abstract fetchDataList(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void;
}
